...
export default Effect.flatMap(SqlClient.SqlClient, (sql) =>
  Effect.Do.pipe(
    ...
    Effect.tap(
      () =>
        sql`ALTER TABLE channel_sync_events ADD CONSTRAINT channel_sync_events_event_type_check CHECK (event_type IN ('channel_created', 'channel_deleted', 'member_added', 'member_removed', 'channel_archived'))`,
    ),
  ),
);
