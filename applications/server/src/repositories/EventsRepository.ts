...
export class EventsRepository extends Effect.Service<EventsRepository>()('api/EventsRepository', {
  ...
  findEventByIdWithDetails = (eventId: Event.EventId) =>
    this.findByIdWithDetails(eventId).pipe(
      Effect.catchTag('SqlError', 'ParseError', LogicError.dieFrom),
    );
  ...
  cancelEvent = (eventId: Event.EventId) =>
    this.cancel(eventId).pipe(Effect.catchTag('SqlError', 'ParseError', LogicError.dieFrom));
  ...
  getDiscordMessageId = (eventId: Event.EventId) =>
    this.getDiscordMessage(eventId).pipe(
      Effect.catchTag('SqlError', 'ParseError', LogicError.dieFrom),
    );

  findEventsByChannelId = (channelId: Discord.Snowflake) =>
    this.findByChannelId(channelId).pipe(
      Effect.catchTag('SqlError', 'ParseError', LogicError.dieFrom),
    );

  markReminderSent = (eventId: Event.EventId) =>
    this.markReminder(eventId).pipe(Effect.catchTag('SqlError', 'ParseError', LogicError.dieFrom));

  markTrainingAutoLogged = (eventId: Event.EventId) =>
    this.markAutoLogged(eventId).pipe(
      Effect.catchTag('SqlError', 'ParseError', LogicError.dieFrom),
    );

  findEndedTrainingsForAutoLog = () =>
    this.findEndedTrainings(undefined).pipe(
      Effect.catchTag('SqlError', 'ParseError', LogicError.dieFrom),
    );

  markEventSeriesModified = (eventId: Event.EventId) =>
    this.markModified(eventId).pipe(Effect.catchTag('SqlError', 'ParseError', LogicError.dieFrom));
